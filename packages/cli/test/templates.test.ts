...
describe("templates", () => {
  ...
  it("renders genuinely different documents for each consumer", () => {
    ...
    const cursor = renderCursorRules(entries, intelligentFacts, "en");
    const gemini = renderGeminiMd(entries, intelligentFacts, "en");
  ...
  it("renders one file per prompt template with claude and vscode paths, namespaced by packId", () => {
    ...
    const files = renderPromptFiles("js-ts", templates, promptFacts, "en");
  ...
  it("does not collide when two different packs render the same template id", () => {
    ...
    const jsFiles = renderPromptFiles("js-ts", templates, promptFacts, "en");
    const pyFiles = renderPromptFiles("python", templates, promptFacts, "en");
  ...
  it("omits generic prompts when the repository provides no supporting evidence", () => {
    ...
    expect(renderPromptFiles("js-ts", templates, emptyFacts, "en")).toEqual([]);
  });
});
